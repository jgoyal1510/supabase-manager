...
export async function POST() {
  try {
    ...
    const defaultValues = {
      ...
      password_hash: '$2b$10$kQCvlJVyrXZqThOz9WZ9HuPUnYZEPQaJ7hVRJ7eFdC0sp66p8Kq0e',
      capacity: 50,
    };
...
export async function DELETE() {
  try {
    ...
    const sqlQuery = `DELETE FROM ehs_ebv.profiles WHERE id IN (${profileIds}) RETURNING id, email;`;
    ...
    // Then, delete refresh tokens from ehs_ebv schema
    console.log('Deleting refresh tokens from ehs_ebv schema...');
    ...
    if (refreshTokenError) {
      console.error('Error deleting refresh tokens from ehs_ebv:', refreshTokenError);
    } else {
      console.log('Successfully deleted refresh tokens from ehs_ebv');
    }

    // Also delete refresh tokens from ehs_ar schema (they also reference ehs_ebv.profiles)
