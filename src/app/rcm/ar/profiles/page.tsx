...
const ARProfilesPage = () => {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/rcm/ar/profiles');
  ...
  const handleInsertTestData = async () => {
    ...
    try {
      ...
      const profilesResponse = await fetch('/api/rcm/ar/profiles');
  ...
  const handleUpdatePasswords = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to update passwords');
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">AR Profiles Management</h1>
